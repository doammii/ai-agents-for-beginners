...
- Basic RAG: user query -> DB -> add to LLM context
...
For example, if it’s asked to create a product launch strategy, it doesn’t rely solely on a prompt that spells out the entire research and decision-making workflow. - query analysis & 및 planning
Instead, the agentic model independently decides to:
...
- **Memory & State:** Because the system maintains state and memory across steps, it can recall previous attempts and their outcomes, avoiding repetitive loops and making more informed decisions as it proceeds. (long-term memory!)
정보 검증 및 반복 처리
